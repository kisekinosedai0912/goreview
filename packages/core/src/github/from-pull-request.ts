...
import { enrichChangedFile } from "../enrich";
...
export type FromPullRequestInput = {
	...
	number: number;
	token?: string;
	/** Pre-authenticated client (e.g. GitHub App installation). Wins over token. */
	octokit?: Octokit;
};
...
export async function fromPullRequest(
	...
	const octokit = input.octokit ?? createOctokit(input.token);
